...
def extractive_answer(question, context):
    ...
    # Keep the context bounded and avoid unnecessary long inference inputs.
...
def warm_qa_model():
    """Warm the QA model in the background after the web server is available.

    This prevents the Render health check from waiting for the Hugging Face
    model download while making the first user request faster when warm-up
    finishes before the request arrives.
    """
    try:
        print("Background warm-up: loading QA model...")
        get_qa_pipeline()
        print("Background warm-up: QA model ready.")
    except Exception as exc:
        print(f"Background QA warm-up skipped: {exc}")
...
if __name__ == "__main__":
    ...
    threading.Thread(target=warm_qa_model, daemon=True).start()

    # Keep the process alive after launching Gradio.
    while True:
        time.sleep(60)
